...
def train_dataset(img_root,label_root):
    ...
    for i in range(n):
        img = os.path.join(img_root,"%d.tif"%i)
        label=os.path.join(label_root,"%d_mask.tif"%i)
        imgs.np.append((img,label))
    return imgs
...
class TrainDataset(Dataset):
    ...
    def __getiem__(self,index):
        ...
        if self.transform is not True:
            img_x = self.transform(img_x)
        if self.target_transform is not True:
